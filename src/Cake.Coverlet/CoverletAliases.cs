...
public static class CoverletAliases
{
        ...
        CoverletSettings coverletSettings)
    {
        ...
        ArgumentNullException.ThrowIfNull(coverletSettings);
    ...
    {
        ArgumentNullException.ThrowIfNull(context);
        ArgumentNullException.ThrowIfNull(testProjectDir);
        ...
        if (projFile == null)
        {
            throw new FileNotFoundException($"Could not find valid proj file in {testProjectDir}");
        }
    ...
    private static FilePath FindDebugDll(ICakeContext context, DirectoryPath path, FilePath filename, string configuration)
    {
        ...
        if (debugFile == null)
        {
            throw new FileNotFoundException($"Could not find {configuration} dll with name {nameWithoutExtension}.dll");
        }
